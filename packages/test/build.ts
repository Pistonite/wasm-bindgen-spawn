...
const PANIC_RUNTIMES = [/*"abort", */"unwind"] as const;
type PanicRuntime = (typeof PANIC_RUNTIMES)[number];
const TARGETS = ["no-modules", "web", "nodejs" , "deno", "bundler"] as const;
type Target = (typeof TARGETS)[number];
const HOST = ["node", "browser"] as const;
...
const main = async () => {
    ...
    for (const wasmPack of output) {
        const { profile, panicRuntime, target, dtsHash, jsHash, bgJsHash, bgWasmHash, bgWasmDtsHash } = wasmPack;
...
const runWasmPack = async (panicRuntime: PanicRuntime, profile: Profile, target: Target): Promise<WasmPackOutput> => {
    const dir = getCargoProjectDir(panicRuntime);
    ...
    const command = [
        "build", "-t", target,
        "--no-gitignore", "--no-pack",
        profile === "release" ? "--release" : "--dev",
        "--out-dir", path.join(targetWasmPackDir, triple),
        "--out-name", "example", "--"
    ];
...
const runPostBuild = (panicRuntime: PanicRuntime, profile: Profile, target: Target, host: Host) => {
    ...
    switch (target) {
        case "no-modules": {
            if (host === "node") {
                ...
                fs.writeFileSync(path.join(outDir, "example_esm.js"), `import fs from "node:fs";globalThis.__fs=fs;\n${js}\nexport default wasm_bindgen;`);
                fs.copyFileSync(path.join(targetWasmPackDir, `${profile}-${panicRuntime}-web`, "example.d.ts"), path.join(outDir, "example_esm.d.ts"));
            }
            else {
                fs.copyFileSync(path.join(wasmPackOutDir, "example.js"), path.join(outDir, "example.js"));
                fs.copyFileSync(path.join(frameworkOutDir, "worker_no_modules.js"), path.join(outDir, "worker.js"));
            }
            break;
        }
        case "web": {
    ...
    fs.copyFileSync(path.join(wasmPackOutDir, "example_bg.wasm"), path.join(outDir, "example_bg.wasm"));
}

const runFrameworkBuild = () => {
    // build the frontend driver website that will test the wasm in browsers using playwright
    const sourceDir = path.resolve(import.meta.dirname, "src", "playwright", "driver");
    ...
    child_process.execSync(`bun build --compile --target=browser ${sourceIndexHtml} --outdir=${outDir}`, {
        stdio: "inherit"
    });
    child_process.execSync(`bun build ${path.join(sourceDir, "worker_no_modules.ts")} --outdir=${outDir}`, {
        stdio: "inherit"
    });
}
